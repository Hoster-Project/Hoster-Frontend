...
export default function SettingsCleaningPage() {
  ...
  const { containerRef: chatContainerRef, endRef: chatEndRef } = useChatAutoScroll(
  ...
  return (
    <div className="pb-8 h-full flex flex-col">
      <div className="flex items-center gap-2.5 px-4 py-4 border-b sticky top-0 bg-background z-50">
        ...
        <h1 className="text-lg font-semibold text-black" data-testid="text-cleaning-title">{getTitle()}</h1>
      </div>
                  ...
                  {dedupedSubscriptions.map((sub) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Badge className={getCategoryBadgeClass(sub.status === "ACCEPTED" ? "active" : sub.status, "status")} data-testid={`subscription-status-${sub.id}`}>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          {existingSub && (
                            <Badge className={`${getCategoryBadgeClass(existingSub.status === "ACCEPTED" ? "active" : existingSub.status, "status")} shrink-0`}>
